...
All four apply to each element on its own.
...
Slides a $K \times K$ filter over the input. $\tilde{x}$ is the input padded with $p$ zeros on every side, and $s$ is the stride. For output channel $k$ at position $(i, j)$:

$$
y_{k,i,j} = b_k + \sum_{c} \sum_{u=0}^{K-1} \sum_{v=0}^{K-1} W_{k,c,u,v} \; \tilde{x}_{c,\, s i + u,\, s j + v}
$$
...
and likewise for $W_\text{out}$. The code does this as one matrix multiply: each $K \times K$ patch is flattened into a column ("im2col") and multiplied by the flattened filters.
...
Encodes a token's position $n$ by rotating pairs of features through position-dependent angles. For head width $d$, base $\theta$ and scaling factor $s$, there is one frequency per pair $k = 0, \dots, d/2 - 1$:

$$
f_k = \frac{1}{s \cdot \theta^{2k/d}}
$$

Split $x$ into halves $x^{(1)}$ and $x^{(2)}$. Each pair $\big(x^{(1)}_k, x^{(2)}_k\big)$ is rotated by angle $n f_k$:
...
The code writes the same rotation as `x * cos + rotate_half(x) * sin`, where `rotate_half` maps $[x^{(1)}, x^{(2)}]$ to $[-x^{(2)}, x^{(1)}]$.
...
**1. Scores.** How much each query matches each key. The scale is $1/\sqrt{d}$ by default (Gemma 3 uses its own):

$$
S = \frac{Q K^\top}{\sqrt{d}}
$$

**2. Mask.** Queries are the last $L_q$ positions, so query $i$ sits at position $q = L_k - L_q + i$. It may attend to key position $j$ only if
...
Disallowed scores are set to $-\infty$, which the code approximates with the dtype's most negative value.
...
The softmax runs in float32 whatever the input dtype.
