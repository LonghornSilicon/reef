"""Position encodings, attention biases and rotations without a token table."""
...
def relative_positions(
    query_len: int, key_len: int, device: torch.device
) -> torch.Tensor:
    """Return ``(query_len, key_len)`` key minus query positions."""
...
class RelativePositionBias(nn.Module):
    ...
    def forward(
        ...
        buckets = self.bucket(relative_positions(query_len, key_len, device))
...
class RotaryEmbedding(nn.Module):
    ...
    def __init__(
        ...
        # Pythia and Phi rotate only the leading rotary_dim features.
